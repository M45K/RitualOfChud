#ifndef HAIRDESIGNER_UNITY_PBS_LIGHTING_INCLUDED
#define HAIRDESIGNER_UNITY_PBS_LIGHTING_INCLUDED
...
inline half4 LightingHairDesigner (SurfaceOutputStandardSpecular s, half3 viewDir, UnityGI gi)
{
	viewDir = normalize(viewDir);
	s.Normal = normalize(s.Normal);
	half ndotl = dot(s.Normal, gi.light.dir);	
	//gi.light.ndotl = (_TwoSidedLight == 1)?((ndotl < 0) ? -ndotl : ndotl): gi.light.ndotl;
	//s.Normal *= (ndotl < 0) ? -1 : 1;
	s.Normal = lerp(normalize(-gi.light.dir+ s.Normal), s.Normal, abs(dot(s.Normal, -gi.light.dir)));
	...
	//half4 c2 = UNITY_BRDF_PBS(s.Albedo, s.Specular, oneMinusReflectivity, s.Smoothness, -s.Normal, viewDir, gi.light, gi.indirect);
	//c = NdotL*c - INdotL*c2;
...
/*
//comment this to disable deferred -> no transparency
inline half4 LightingHairDesigner_Deferred (SurfaceOutputStandardSpecular s, half3 viewDir, UnityGI gi, out half4 outDiffuseOcclusion, out half4 outSpecSmoothness, out half4 outNormal)
{
	// energy conservation
	half oneMinusReflectivity;
...
*/
inline void LightingHairDesigner_GI (
